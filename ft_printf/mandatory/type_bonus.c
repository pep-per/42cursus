...
/*   Updated: 2023/01/05 20:23:20 by jiyeolee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					int (*ft_put)(void *p, unsigned int len))
{
	...
	if (tags->hash == 1 && num != 0)
		precise_len += 2;
	if (tags->minus == 0 && (tags->hash == 0 || tags->zero == 0 || tags->precision != -1))
		if (apply_width(tags, precise_len) == -1)
			return (-1);
	if (tags->hash == 1 && num != 0)
	{
	...
	if (tags->minus == 0 && tags->hash == 1 && tags->zero == 1 && tags->precision == -1)
		if (apply_width(tags, precise_len) == -1)
			return (-1);
	if (apply_precision(tags, len) == -1)
		return (-1);
